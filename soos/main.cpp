...
#define errfail(wut) \
{ \
printf(#wut " fail (line #%03i): (%i) %s\n", __LINE__, errno, strerror(errno)); \
killswitch(); \
}
#define errtga(wut) { printf(#wut " fail (line #%03i): (%i) %s\n", __LINE__, res, tga_error(res)); killswitch(); }
#define errjpeg() { printf("JPEG fail (line #%03i): %s\n", __LINE__, tjGetErrorStr()); killswitch(); }
...
u8 source_buf[256 * 400 * 4 * 2];
u8 decode_buf[256 * 400 * 4];
int srcfmt[2] = {3, 3};
...
void killswitch() {
    ...
    if (screens[0]) SDL_FreeSurface(screens[0]);
    if (screens[1]) SDL_FreeSurface(screens[1]);
...
int main(int argc, char *argv[]) {
    ...
    win = SDL_CreateWindow("HorizonScreen " BUILDTIME, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 720, 240,
    ...
    screens[0] = mksurface(240, 400, 3, 1);
    screens[1] = mksurface(240, 320, 3, 1);
    ...
    do {
        u32 *_1 = (u32 *) source_buf;

        int i, j, k;
        for (i = 0; i != 0x100; i++)
            for (j = 0; j != 8; j++)
                for (k = 0; k != 8; k++)
                    if (ctrufont_bin[(i << 3) + j] &
                        (1 << (~k & 7))) {
                        _1[((i >> 4) << 10) + ((i & 0xF) << 3) + (j << 7) + k] = INT_MAX;
                    } else {
                        _1[((i >> 4) << 10) + ((i & 0xF) << 3) + (j << 7) + k] = 0;
                    }

        SDL_UpdateTexture(font, nullptr, source_buf, 128 * 4);
        SDL_SetTextureBlendMode(font, SDL_BLENDMODE_BLEND);

        i = sizeof(source_buf) >> 2;
        _1 = (u32 *) source_buf;
        while (i--) {
            *(_1++) = rand();
        }
    ...
    while (PumpEvent()) {
        ...
        switch (p->packetid) {
            ...
            case 0x02: //MODESET
                ...
                screens[0] = mksurface(stride[0] / bsiz[0], 400, bsiz[0], srcfmt[0]);
                screens[1] = mksurface(stride[1] / bsiz[1], 320, bsiz[1], srcfmt[1]);
                break;

            case 0x03: //DATA_TGA
            {
                tga.image_data = decode_buf;
                ...
                memcpy(source_buf + offs, decode_buf, tga.height * stride[tga.origin_y / 400]);
            ...
            case 0x04: //DATA_JPEG
            {
                ...
                tjDecompress2(jdec, &p->data[8], p->size - 8, source_buf + offs, iw, 0, ih,
                              (srcfmt[*(u32 *) &p->data[0] / 400] & 1) ? TJPF_RGB : TJPF_RGBA,
                              TJFLAG_FASTUPSAMPLE | TJFLAG_NOREALLOC | TJFLAG_FASTDCT);
        ...
        if (screens[0]) {
            SDL_LockSurface(screens[0]);
            memcpy(screens[0]->pixels, source_buf, stride[0] * 400);
            SDL_UnlockSurface(screens[0]);

            SDL_DestroyTexture(tex[0]);
            tex[0] = SDL_CreateTextureFromSurface(rendertop, screens[0]);
        } else puts("img[0] nullptr!");

        if (screens[1]) {
            SDL_LockSurface(screens[1]);
            memcpy(screens[1]->pixels, source_buf + (stride[0] * 400), stride[1] * 320);
            SDL_UnlockSurface(screens[1]);

            SDL_DestroyTexture(tex[1]);
            tex[1] = SDL_CreateTextureFromSurface(rendertop, screens[1]);
