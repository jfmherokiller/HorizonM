...
#define SANE_DEPTH(x) ((x) == 8 || (x) == 16 || (x) == 17 || (x) == 18 || (x) == 24 || (x) == 32)
#define UNMAP_DEPTH(x)            ((x) == 16 || (x) == 17 || (x) == 18 || (x) == 24 || (x) == 32)
...
/* helpers */
static tga_result tga_read_rle(tga_image *dest, uint8_t *fp);
static tga_result tga_write_row_RLE(uint8_t **fp,
...
static uint16_t* postincrement16(uint8_t** ptr)
{
...
const char *tga_error(const tga_result errcode)
{
    ...
    case TGAERR_2SMOL:
        return "target buffer size too small";
    case TGAERR_EINVAL:
 ...
 *          valid.
 */
tga_result tga_read_from_FILE(tga_image *dest, uint8_t *fp)
{
    #define BARF(errcode) \
        { tga_free_buffers(dest);  return errcode; }

    #define READ(destptr, size) \
        { memcpy(destptr, fp, size); fp += size; }

    #define READ16(dest) \
        dest = *postincrement16(&fp);

    dest->image_id = NULL;
    dest->color_map_data = NULL;
    
    if(!dest->image_data) BARF(TGAERR_EINVAL);
 ...
 *          to be valid.
 */
tga_result tga_write_to_FILE(uint8_t *fp, const tga_image *src, int *fsiz)
{
    #define WRITE(srcptr, size) \
        { memcpy(fp, srcptr, size); fp += size; }

    #define WRITE16(src) \
        *postincrement16(&fp) = src
 ...
 * tga_image struct.
 */
void init_tga_image(tga_image *img, uint8_t *image,
...
/*
tga_result tga_write_rgb_rle(uint8_t *filename, uint8_t *image,
    const uint16_t width, const uint16_t height, const uint8_t depth)
{
    tga_image img;
    ...
    case 17:
        {
            ...
            #define FIVE_BITS (BIT(0)|BIT(1)|BIT(2)|BIT(3)|BIT(4))
            if (b) *b = ((src16      ) & FIVE_BITS) << 3;
            if (g) *g = ((src16 >>  5) & (FIVE_BITS|BIT(5))) << 2;
            if (r) *r = ((src16 >> 11) & FIVE_BITS) << 3;
            if (a) *a = 0;
            #undef FIVE_BITS
            break;
        }
    case 18:
        {
            ...
            #define FIVE_BITS (BIT(0)|BIT(1)|BIT(2)|BIT(3))
            if (b) *b = ((src16      ) & FIVE_BITS) << 4;
            if (g) *g = ((src16 >>  4) & FIVE_BITS) << 4;
            if (r) *r = ((src16 >>  8) & FIVE_BITS) << 4;
            if (a) *a = ((src16 >> 12) & FIVE_BITS) << 4;
            #undef FIVE_BITS
            break;
        }
    ...
    case 17:
        {
            ...
            #define FIVE_BITS (BIT(0)|BIT(1)|BIT(2)|BIT(3)|BIT(4))
            tmp |=  (b >> 3) & FIVE_BITS;
            tmp |= ((g >> 2) & (FIVE_BITS|BIT(5))) << 5;
            tmp |= ((r >> 3) & FIVE_BITS) << 11;
            #undef FIVE_BITS
